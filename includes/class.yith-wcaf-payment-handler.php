...
if ( ! class_exists( 'YITH_WCAF_Payment_Handler' ) ) {
	...
	class YITH_WCAF_Payment_Handler {
		...
		public function get_readable_status( $status ) {
			return apply_filters( "yith_wcaf_{$status}_payment_status_name", ucfirst( str_replace( '-', ' ', $status ) ) );
		}
		...
		public function add_screen_option() {
			if ( 'yit-plugins_page_yith_wcaf_panel' == get_current_screen()->id && isset( $_GET['tab'] ) && $_GET['tab'] == 'payments' ) {
