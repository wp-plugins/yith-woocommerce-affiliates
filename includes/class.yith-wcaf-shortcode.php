	...
	class YITH_WCAF_Shortcode {
		...
		public static function link_generator( $atts = array() ){
			...
			if( $original_url ) {
				...
				if ( $parsed_original_url['host'] != $_SERVER['SERVER_NAME'] ) {
					$original_url = false;
				}
			}
		...
		public static function affiliate_dashboard(){
			...
			if( ! YITH_WCAF_Affiliate_Handler()->is_user_enabled_affiliate() ){
				...
				return self::registration_form( array(
			...
			if ( isset( $wp->query_vars['settings'] ) ) {
				return self::affiliate_dashboard_settings();
			}

			// otherwise, show summary
			return self::affiliate_dashboard_summary();
		}
