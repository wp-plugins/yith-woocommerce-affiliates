...
if ( ! class_exists( 'YITH_WCAF_Affiliate_Handler' ) ) {
	...
	class YITH_WCAF_Affiliate_Handler {
		...
		public function get_affiliates_via_ajax() {
			...
			$found_affiliates_raw = $this->get_affiliates( array( 'user_login' => $term, 'user_email' => $term ) );
		...
		public function add_screen_option() {
			if ( 'yit-plugins_page_yith_wcaf_panel' == get_current_screen()->id && isset( $_GET['tab'] ) && $_GET['tab'] == 'affiliates' ) {
		...
		public function render_affiliate_extra_fields( $user ) {
			...
			?>
			<hr />
			<h3><?php _e( 'Affiliate informations', 'yith-wcaf' )?></h3>
